...
static int _reset;
static char *modelname = "Fast inactivating voltage gated potassium current";
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
   double _lalpham , _lbetam , _lalphah , _lbetah , _lQ10 ;
 _lQ10 = pow( 3.0 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   _lalpham = 1.4 / ( 1.0 + exp ( ( _lv + 27.0 ) / ( - 12.0 ) ) ) ;
   _lbetam = 0.49 / ( 1.0 + exp ( ( _lv + 30.0 ) / 4.0 ) ) ;
   taum = 1.0 / ( _lQ10 * ( _lalpham + _lbetam ) ) ;
   minf = _lalpham / ( _lalpham + _lbetam ) ;
   _lalphah = 0.0175 / ( 1.0 + exp ( ( _lv + 50.0 ) / 8.0 ) ) ;
   _lbetah = 1.3 / ( 1.0 + exp ( ( _lv + 13.0 ) / ( - 10.0 ) ) ) ;
   tauh = 1.0 / ( _lQ10 * ( _lalphah + _lbetah ) ) ;
